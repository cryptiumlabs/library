...
## Tezos是什么?
Tezos是一个公共分布式账本，或称为区块链，它是一个主要用OCaml函数式编程语言编写的开源项目。 除了分布式记账以外，它还是一个智能合约平台，支持用Michelson语言编写的合约。 Michelson是强类型的，就像OCaml一样，适用于编写便于形式验证的智能合约（有助于提高系统的正确性，及时发现合约漏洞，例如臭名昭着的DAO或Multi-Sig Wallet）。 Michelson仅限于编写与区块链相关的常见应用程序，例如 multisig wallets，vesting 或者 distribution rules，旨在更容易地检查，究竟是什么命令被执行。
